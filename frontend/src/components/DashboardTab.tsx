...
import { Zap, Droplets, Sparkles, TrendingUp, TrendingDown, Minus, ChevronRight, ChevronLeft, ChevronRight as ChevronRightIcon } from 'lucide-react';
...
export default function DashboardTab({ desktop = false, userRole = 'user', userMaHo = null }: Props) {
  ...
  // Admin: có thể chọn phòng bất kỳ; user: chỉ xem phòng của mình
  const defaultHo = isAdmin
    ? hoGiaDinhList[0].MaHo
    : (userMaHo ?? hoGiaDinhList[0].MaHo);

  const [selectedHo, setSelectedHo] = useState(defaultHo);

  // Reset về phòng đúng khi user thay đổi (nếu không phải admin)
  const effectiveHo = isAdmin ? selectedHo : (userMaHo ?? hoGiaDinhList[0].MaHo);

  const ho = hoGiaDinhList.find(h => h.MaHo === effectiveHo) ?? hoGiaDinhList[0];
  const hoaDon = hoaDonList.find(hd => hd.MaHo === effectiveHo && hd.ThangNam === CURRENT_PERIOD);

  // Tiêu thụ kỳ hiện tại
  const dienTieuThu = getTieuThu(effectiveHo, 'Dien', CURRENT_PERIOD);
  const nuocTieuThu = getTieuThu(effectiveHo, 'Nuoc', CURRENT_PERIOD);
  const dienDonGia = dongHoList.find(d => d.MaHo === effectiveHo && d.Loai === 'Dien')?.DonGia || 3000;
  const nuocDonGia = dongHoList.find(d => d.MaHo === effectiveHo && d.Loai === 'Nuoc')?.DonGia || 15000;
  const tongTamTinh = dienTieuThu * dienDonGia + nuocTieuThu * nuocDonGia;

  // Tiêu thụ tháng trước để tính trend
  const dienPrev = getTieuThu(effectiveHo, 'Dien', '2026-02');
  const nuocPrev = getTieuThu(effectiveHo, 'Nuoc', '2026-02');
  ...
  // Chart data — 3 tháng
  const dien3m = getLast3Months(effectiveHo, 'Dien');
  const nuoc3m = getLast3Months(effectiveHo, 'Nuoc');
  ...
  // Tổng hợp tất cả phòng cho admin summary
  ...
  // Cảnh báo nổi bật — admin thấy tất cả, user chỉ thấy của phòng mình
  const alertsAll = phanTichAIList
    .filter(a => {
      if (a.MucDoCanhBao === 'normal') return false;
      if (!isAdmin) {
        const hd = hoaDonList.find(h => h.MaHoaDon === a.MaHoaDon);
        return hd?.MaHo === effectiveHo;
      }
      return true;
    })
    .sort((a, b) => (a.MucDoCanhBao === 'danger' ? -1 : 1));

  // Chuyển phòng prev/next cho admin
  const currentHoIndex = hoGiaDinhList.findIndex(h => h.MaHo === effectiveHo);
  const handlePrevHo = () => {
    const prev = hoGiaDinhList[(currentHoIndex - 1 + hoGiaDinhList.length) % hoGiaDinhList.length];
    setSelectedHo(prev.MaHo);
  };
  const handleNextHo = () => {
    const next = hoGiaDinhList[(currentHoIndex + 1) % hoGiaDinhList.length];
    setSelectedHo(next.MaHo);
  };
  ...
  // ── Selector phòng cho Admin ──────────────────────────────────────
  function RoomSelector({ compact = false }: { compact?: boolean }) {
    if (!isAdmin) return null;
    return (
      <div className={`flex items-center gap-2 ${compact ? '' : 'mb-4'}`}>
        <button
          onClick={handlePrevHo}
  ...
  if (desktop) {
    return (
      <div className="space-y-6">
        {/* ── Summary Cards Grid — 4 col on desktop (Admin only) ── */}
        {isAdmin && (
        ...
        <div className="grid grid-cols-5 gap-4">
            ...
            {isAdmin && (
              <div className="bg-white rounded-2xl px-4 py-3 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-400 font-medium">Chọn phòng xem chi tiết</span>
                  <RoomSelector compact />
                </div>
              </div>
            )}
          ...
          <div className="col-span-2">
            {alertsAll.length > 0 ? (
              ...
              /* Không có cảnh báo */
              <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-800 via-slate-900 to-slate-950 p-5 shadow-lg h-full flex items-center justify-center">
                <div className="text-center">
                  <div className="w-12 h-12 rounded-2xl bg-emerald-500/20 flex items-center justify-center mx-auto mb-3">
                    <Sparkles size={22} className="text-emerald-400" />
                  </div>
                  <p className="text-sm font-semibold text-white">Không có cảnh báo</p>
                  <p className="text-[11px] text-white/50 mt-1">Tiêu thụ bình thường</p>
                </div>
              </div>
            )}
          </div>
  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      {/* ── Room Selector (Admin only — Mobile) ────────────── */}
      {isAdmin && (
        <div className="mx-4 bg-white rounded-2xl px-4 py-3 shadow-sm border border-gray-50">
          <p className="text-[10px] text-gray-400 font-medium mb-2">Chọn phòng xem chi tiết</p>
          <div className="flex items-center gap-2">
            <button onClick={handlePrevHo} className="w-7 h-7 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors">
              <ChevronLeft size={14} className="text-gray-600" />
            </button>
            ...
            <button onClick={handleNextHo} className="w-7 h-7 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors">
              <ChevronRightIcon size={14} className="text-gray-600" />
            </button>
          </div>
        </div>
