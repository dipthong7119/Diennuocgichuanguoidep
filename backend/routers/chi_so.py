...
def _tinh_tong_tien(chi_so_cu: int, chi_so_moi: int, don_gia: float,
                     thay_dong_ho: bool = False) -> float:
    """
    Tính TongTien = (ChiSoMoi - ChiSoCu) * DonGia.

    Trường hợp ThayDongHo=True (đồng hồ vừa được thay/reset), ChiSoMoi có thể
    NHỎ HƠN ChiSoCu vì đó là số đọc trên đồng hồ mới — khi đó tiêu thụ kỳ này
    tính trực tiếp từ ChiSoMoi (coi như đồng hồ mới bắt đầu từ 0), không lấy hiệu.
    """
    if thay_dong_ho and chi_so_moi < chi_so_cu:
        return chi_so_moi * don_gia
...
def nhap_chi_so(
    ...
    try:
        ...
        # 3. Lưu chỉ số (ThayDongHo chỉ dùng để tính tiền, không lưu vào bảng ChiSoTieuThu)
        chi_so_data = payload.model_dump(exclude={"ThayDongHo"})
        chi_so = ChiSoTieuThu(**chi_so_data)
        db.add(chi_so)

        # 4. Tính TongTien (xử lý riêng nếu vừa thay đồng hồ mới)
        tong_tien = _tinh_tong_tien(
            payload.ChiSoCu, payload.ChiSoMoi, dong_ho.DonGia, payload.ThayDongHo
        )
