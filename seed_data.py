...
def seed():
    ...
    try:
        ...
        if not db.query(NguoiDung).first():
            db.add_all([
                # admin: MaHo = None → có thể xem tất cả
                NguoiDung(Username="admin", PasswordHash=hash_pw("admin123"), Role="admin", MaHo=None),
                # user1 → phòng HO-001 (Nguyễn Văn An, P101)
                NguoiDung(Username="user1", PasswordHash=hash_pw("user123"), Role="user", MaHo="HO-001"),
                # user2 → phòng HO-002 (Trần Thị Bình, P102)
                NguoiDung(Username="user2", PasswordHash=hash_pw("user456"), Role="user", MaHo="HO-002"),
                # user3 → phòng HO-003 (Lê Minh Cường, P201)
                NguoiDung(Username="user3", PasswordHash=hash_pw("user789"), Role="user", MaHo="HO-003"),
            ])
            print("[+] Đã tạo 4 tài khoản: admin, user1, user2, user3")
        ...
        print("  User 1: user1  / user123   → chỉ xem/thanh toán phòng HO-001 (P101)")
        print("  User 2: user2  / user456   → chỉ xem/thanh toán phòng HO-002 (P102)")
        print("  User 3: user3  / user789   → chỉ xem/thanh toán phòng HO-003 (P201)")
        print("=" * 50)
